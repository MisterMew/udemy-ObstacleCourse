...
public class OscillateObject : MonoBehaviour
{
    ...
    //Speed Control
    [SerializeField] [Range(0f, 10f)] private float frequencyMultiplier = 1f;
    ...
    [SerializeField] [Range(0f, 1000f)] private float amplitudeMultiplier = 1f;
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawLine(GizVec(1, -displacement.x), GizVec(1, displacement.x));
        Gizmos.color = Color.green;
        Gizmos.DrawLine(GizVec(2, -displacement.y), GizVec(2, displacement.y));
        Gizmos.color = Color.blue;
        Gizmos.DrawLine(GizVec(3, -displacement.z), GizVec(3, displacement.z));
    }

    private Vector3 GizVec(int xyz, float displacement)
    {
        if (xyz == 1) return new Vector3(displacement, transform.position.y, transform.position.z);
        if (xyz == 2) return new Vector3(transform.position.x, displacement, transform.position.z);
        if (xyz == 3) return new Vector3(transform.position.x, transform.position.y, displacement);
